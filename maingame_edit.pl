...
entries_loop([]):- !.
entries_loop([H|T]) :- assertz(databasefile(H)),assertz(loadedfiles(H)), entries_loop(T),listing(loadedfiles).
main_1:-   
    assert(loadingstatus(0)), % put the current directry here
    directory_files("/home/garvitjoshi/Desktop/game/prolog uni/databses",Entries),
    write(Entries),
    entries_loop(Entries).
